...
def popular_movies():
    ...
    for index, row in output.iterrows():
        temp_list={ "original_title": row['original_title'], 
                    "poster_link": row['poster_link'], 
                    "release_date": row['release_date'] or "N/A", 
                    "duration": row['runtime'], 
                    "rating": row['weighted_rating']/2 } 
        
        popular_movie_data.append(temp_list) 
        return jsonify({ "data": popular_movie_data, 
                         "status": "success" })
...
def recommended_movies():
    ...
    for index, row in all_recommended.iterrows(): 
        ...
        recommended_movie_data.append(temp_list) 

    return jsonify({ "data":recommended_movie_data, 
                     "status": "success" })
